...
<script type="text/javascript">
	...
	$(function () {
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/notice/LaterNoticeList",
			success:function (msg) {
				if(msg.map.statusCode == 200){
				    $(msg.map.noticeList).each(function (index,item) {
						var li = "<li class='ue-clear'>"
                            		+"<span>"+moment(item.ndate).format('YYYY-MM-DD')+"</span>&nbsp;&nbsp;&nbsp;<a onclick='showWindow("+item.nid+")' href='#' class='notice-title'>"+item.ntitle+"</a>"
						          +"</li><p>";
	...
	function showWindow(nid) {
	    $.ajax({
			...
			success:function (msg) {
                ...
                $("#content").text(msg.remark).css({"font-size":"20px","font-weight":"bold"});
                $("#showdiv").show();
    ...
    $(function () {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/fatie/fatieList",
            success:function (msg) {
                if(msg.map.statusCode == 200){
                    $(msg.map.forumPostList).each(function (index,item) {
                        var li = "<li class='ue-clear'>"
                            +"<span>"+moment(item.createtime).format('YYYY-MM-DD')+"</span>&nbsp;&nbsp;&nbsp;<a onclick='showWindow1("+item.forumid+")' href='#' class='notice-title'>"+item.forumtitle+"</a>"
                            +"</li><p>";
                        $("#tiezi-list").append(li);
                    })
                }
            }
        })
    });
    function showWindow1(forumid) {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/fatie/info/"+forumid,
            success:function (msg) {
                $("#ntitle").text(msg.forumtitle);
                $("#content").text(msg.forumcontent).css({"font-size":"20px","font-weight":"bold"});
                $("#showdiv").show();
                $("#cover").css({"height":document.body.scrollHeight+"px"});
                $("#cover").show();
            }
...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
    ...
    <table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr bgcolor="#FFFFFF">
			<td>
				<ul id="tiezi-list" class="notice-list">
